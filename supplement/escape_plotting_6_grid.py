...
for EscapeFunction in EscapeFunctionMatrix:
    ...
    Escape_X7, Escape_Y7    = Escape_Plot(7,  EscapeFunction, MaxEscapeRateDay14, EscapeFunctionSlope) ### modified on 7/30/2021
...
for i, row in enumerate(axs):
    for j, cell in enumerate(row):
        ...
        if j == 0:
            ...
            cell.plot(X7list[i],  Y7list[i],   color=color_for_7,  lw=2.0)
            cell.plot(X10list[i], Y10list[i],  color=color_for_10, lw=2.0)
            cell.plot(X14list[i], Y14list[i],  color=color_for_14, lw=2.0)
