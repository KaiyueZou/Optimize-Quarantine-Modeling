...
05/10/2022
- change the 7-day length to 5 days 
...
for EscapeFunctionNum in EscapeFunctionNumVector:
    ...
    SevenDayQuaARMatrix     = np.genfromtxt(FileName1, delimiter = ',')
    TenDayQuaARMatrix       = np.genfromtxt(FileName2, delimiter = ',')
    FourteenDayQuaARMatrix  = np.genfromtxt(FileName3, delimiter = ',')
        ...
        for j, cell in enumerate(row):
            ...
            if i == 0:
                ...
                if EscapeFunctionNum == 101: 
                    cell.axis([-0.03, 1.03, -2, 50])
                elif EscapeFunctionNum == 102:
                    cell.axis([-0.03, 1.03, -2, 50])
                elif EscapeFunctionNum == 103:
                    cell.axis([-0.03, 1.03, -2, 50])
                cell.set_aspect('auto')
                cell.plot(X_Axis_real, y7*100,   linestyle=ls_for_7,  color="black", alpha = 1.0, lw=2)
                cell.plot(X_Axis_real, y10*100,  linestyle=ls_for_10,  color="black", alpha = 0.7, lw=2)
                cell.plot(X_Axis_real, y14*100,  linestyle=ls_for_14,  color="black", alpha = 0.5, lw=2)
                ...
                cell.set_aspect('auto')
                if EscapeFunctionNum == 101: 
                    cell.axis([-0.03, 1.03, -0.2, 0.2])
                else:
                    cell.axis([-0.03, 1.03, -0.005, 0.005])
                (markers, stemlines, baseline) =  cell.stem(X_Axis_real, y10 - y14)
                if x14_10 in X_Axis_real:
                    cell.axvline(x = x14_10, color = 'grey', lw=1.0, linestyle=(0, (3, 5, 1, 5)))
                pl.setp(stemlines, linestyle="-", color="black", linewidth=2 )
                pl.setp(markers, color="black", markersize=4)
                pl.setp(baseline, linestyle="--", color="black", linewidth=0.5)
